...
**`project-meta-strip`**
- Use for the Period / Position / Role row under a project-detail hero.
- Layout: 3 equal columns inside the main reading column, centered text, no card background, no outer border, no shadow.
- Label: `{typography.caption-sm}` with uppercase tracking and `{colors.body}`.
- Value: `{typography.body-sm-strong}` in `{colors.ink}`, vertically centered within a stable row height.
- Divider: keep only the Position column with `1px solid {colors.hairline}` on both left and right sides. Do not add borders to Period or Role, because that creates an outer frame instead of a quiet internal separator.
...
- Prefer document-style sections over card UI, especially on portfolio and project-detail pages. Use whitespace, typography, and occasional `{colors.hairline}` internal separators before adding a bordered container.
...
- Don't wrap ordinary content blocks such as overview, role, skills, achievements, or project descriptions in card UI. Use cards only when the content is a repeated comparable item, a true product preview, or a deliberately framed tool surface.
