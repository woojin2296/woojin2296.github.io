    ...
    <ProjectPageLayout sectionLinks={sectionLinks}>
      ...
      <ContentSection id="overview">
        ...
        <ContentText>
          ...
          **실시간 관제 및 이상 알림 시스템**입니다. 기존 환경에서는 수집이 멈추거나
          데이터 품질에 문제가 생겨도 직접 데이터를 열람하기 전까지 이상 여부를 확인하기 어려웠고,
          실제로 **데이터 수집 중단을 3일간 인지하지 못한 사례**가 있었습니다.
          ...
          약 **1년 8개월**간 연구 과제 환경에서 실제 운영했으며, 현재도 연구실 실험 데이터
      ...
      <ContentSection id="monitoring-architecture">
        ...
        <ContentText>
          ...
          감지 대상이 늘어나더라도 해당 대상의 **checker 모듈을 추가**하는 방식으로
          쉽게 확장할 수 있도록 구조를 잡았습니다. Slack 외의 알림 채널이 필요하면
          **notifier 모듈을 추가**하는 구조로 설계했습니다. 백엔드는 Watchdog이
          보낸 감지 결과를 저장하고, 프론트엔드는 저장된 상태 이벤트를 조회하거나
          **WebSocket**으로 전달받아 화면을 갱신합니다.
          `}
        </ContentText>
        <ContentFigureGroup
          figures={[
      ...
      <ContentSection id="realtime-delivery">
        ...
        <ContentText>
          {`
          Watchdog은 각 감지 대상에 맞는 checker 전략을 실행하고 이상 상태를 판단합니다.
          이상 상태가 확인되면 Watchdog은 감지 결과를 **TCP 기반 소켓 통신**으로 Spring Boot
          백엔드에 전달합니다. 백엔드는 감지 결과를 상태 이벤트로 저장한 뒤 **WebSocket 이벤트**로
          변환해 Next.js 프론트엔드에 전달합니다. Slack 알림도 같은 상태 이벤트를 기준으로
          전송되도록 연동했습니다.
          `}
        </ContentText>
        <ContentFigureGroup
      ...
      <ContentSection id="deployment-automation">
        ...
        <ContentText>
          {`
          서버가 **학교 내부망**에 있어 외부에서 SSH로 접속하는 배포 파이프라인을 사용할 수 없었습니다.
          내부망 서버에 **self-hosted runner**를 설치하고, GitHub Actions job이 해당
          runner에서 실행되도록 구성했습니다. GitHub Actions는 내부망 runner에 job을 전달하고,
          runner가 서버 안에서 소스 갱신, Docker 이미지 빌드, Docker Compose 기반 컨테이너
          재배포를 순서대로 수행하도록 구성했습니다.
          `}
        </ContentText>
        <ContentFigureGroup
          figures={[
