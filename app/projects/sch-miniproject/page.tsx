  ...
  return (
    ...
    >
      ...
      <ContentSection id="overview">
        ...
        <ContentText>
          {`
          순천향대학교 사물인터넷학과 머신러닝·딥러닝 강의에서 사용하는 **프로젝트 과제 관리 시스템**입니다.
          ...
          구현했습니다. 이 시스템은 졸업 전까지 **10개월**간 직접 운영했으며, 현재도 강의 운영 환경에서
          계속 사용되고 있습니다. 현재 **월 평균 약 40명의 학생**이 과제 제출과 랭킹 조회 기능을 이용하고 있습니다.
          `}
        </ContentText>
      </ContentSection>
      ...
      <ContentSection id="pms-workflow">
        ...
        <ContentText>
          {`
          사용자는 프로젝트 결과와 점수를 웹으로 제출하고 제출 후 바로 자신의 랭킹을 확인할
          수 있도록 구성했습니다. 교수자는 관리자 페이지에서 사용자 관리와 제출 점수 집계를
          처리하고, 프로젝트별 랭킹을 한 번에 확인할 수 있습니다. 전체 랭킹은 엑셀 형식으로
          출력해 성적 정리 과정에 바로 활용할 수 있도록 구성했습니다.
          `}
        </ContentText>
        <ContentFigureGroup
          figures={[
      ...
      <ContentSection id="data-model">
        ...
        <ContentText>
          {`
          강의 서버에서는 **Next.js** 애플리케이션, **SQLite DB**, 제출 파일 저장 영역을
          Docker Compose 환경에서 함께 운영했습니다. 월 평균 약 40명 규모의 강의
          운영 시스템이라 별도 DB 서버를 두기보다 SQLite로 관리 비용을 낮췄고, DB
          파일과 제출 파일은 각각 **별도 볼륨으로 분리**해 컨테이너 재시작 이후에도 데이터가
          유지되도록 했습니다.
          `}
        </ContentText>
        <ContentFigureGroup
      ...
      <ContentSection id="auth-authorization">
        ...
        <ContentText>
          {`
          강의 운영 시스템은 외부 서비스 연동보다 학생·관리자 권한을 서버에서 확실하게
          통제하는 것이 중요했습니다. 그래서 토큰을 클라이언트에서 직접 다루는 방식보다
          서버가 로그인 상태와 권한을 관리하는 **세션 기반 인증**을 선택했습니다. 인증은
          학번 기반 로그인과 session_token 쿠키, 서버 세션을 기준으로 처리했습니다.
          보호 경로에서는 세션을 먼저 확인하고, 학생용 API와 관리자용 API를 경로
          기준으로 분리해 역할에 맞는 요청만 처리되도록 했습니다. 학생은 본인의 제출
          결과와 랭킹 조회 기능에 접근하고, 관리자는 사용자 관리, 점수 집계, 제출
          파일 확인 같은 운영 기능에 접근할 수 있도록 인가 범위를 나눴습니다.
          `}
        </ContentText>
        <ContentFigureGroup
          columns={1}
      ...
      <ContentSection id="security-validation">
        ...
        <ContentText>
          {`
          제출 파일은 성적 처리의 근거가 되기 때문에 단순 첨부 파일이 아니라 검증 대상 데이터로 다뤘습니다.
          입력 단계, 저장 단계, 다운로드 단계에서 필요한 확인을 나눠 적용했습니다.
          `}
        </ContentText>
        <ContentSubTitle>프론트엔드 검증</ContentSubTitle>
        <ContentText>
          {`
          프론트엔드에서는 빈 파일을 먼저 차단하고, 확장자 제한을 적용해 .ipynb, .py 파일만
          선택할 수 있게 했습니다. 이를 통해 서버 요청 전에 사용자 입력 오류를 줄였습니다.
          `}
        </ContentText>
        <ContentSubTitle>서버 검증</ContentSubTitle>
        <ContentText>
          {`
          인증된 세션이 있는 사용자만 제출할 수 있게 하고, 빈 파일과 10MB 초과 파일을
          서버에서 차단했습니다. 업로드 확장자는 .ipynb, .py만 허용하고, 파일명 특수문자를
          정리한 뒤 randomUUID를 붙여 저장 파일명 충돌을 방지했습니다.
          `}
        </ContentText>
        <ContentText>
          {`
          파일은 사용자 ID별 디렉터리에 저장했고, 저장 전 업로드 루트와 저장
          대상 경로를 각각 절대 경로로 변환한 뒤, 최종 경로가 업로드 루트 내부에서
          시작하는지 확인했습니다. .ipynb는 JSON 파싱 후 cells 배열 존재 여부를
          확인하고, .py는 null byte 포함 여부를 검사했습니다.
          `}
        </ContentText>
      ...
      <ContentSection id="audit-logging">
        ...
        <ContentText>
          {`
          성적 반영에는 점수뿐 아니라 제출 시간도 중요했기 때문에, 제출 실패, 로그인 실패,
          점수 관리, 관리자 작업처럼 사후 확인이 필요한 요청을 request_logs 테이블에
          남겼습니다. 로그에는 요청자, 경로, 메서드, 상태 코드, IP, 요청 메타데이터를
          포함했고, 관리자 페이지에서는 요청 경로, method, status, IP, metadata
          기준 검색과 페이지네이션으로 점수 처리와 제출 과정의 문제를 추적하도록 구성했습니다.
          `}
        </ContentText>
        <ContentFigureGroup
          figures={[
