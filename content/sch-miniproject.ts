...
export const sections = [
  ...
  },
  {
    ...
    paragraphs: [
      "강의 서버에서는 **Next.js** 애플리케이션, **SQLite DB**, 제출 파일 저장 영역을 Docker Compose 환경에서 함께 운영했습니다. 월 평균 약 40명 규모의 강의 운영 시스템이라 별도 DB 서버를 두기보다 SQLite로 관리 비용을 낮췄고, DB 파일과 제출 파일은 각각 **별도 볼륨으로 분리**해 컨테이너 재시작 이후에도 데이터가 유지되도록 했습니다.",
    ],
    figures: [
      {
        src: "/projects/sch-iot-rankingboard/sch-miniproject-system-architecture.png",
  ...
  },
  {
    ...
    paragraphs: [
      "강의 운영 시스템은 외부 서비스 연동보다 학생·관리자 권한을 서버에서 확실하게 통제하는 것이 중요했습니다. 그래서 토큰을 클라이언트에서 직접 다루는 방식보다 서버가 로그인 상태와 권한을 관리하는 **세션 기반 인증**을 선택했습니다. 인증은 학번 기반 로그인과 session_token 쿠키, 서버 세션을 기준으로 처리했습니다.",
